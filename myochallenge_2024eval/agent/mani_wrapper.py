...
class ManiWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if self.sim_step < 200:
            self.target_obj_pos = self.obs_dict['Rpalm_pos'] + np.array([0, 0, 0.10])
        else:
            self.target_obj_pos = self.obs_dict['Rpalm_pos'] + np.array([0, 0, 0.04])
        ...
        if self.sim_step < 450 and (self.obs_dict['obj_pos'][1] - self.target_obj_pos[1] > 0.03):
            terminated = True
            truncated = True
